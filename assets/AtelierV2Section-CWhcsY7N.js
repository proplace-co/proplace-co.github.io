...
`)}),S(!0))},[]);f.useEffect(()=>{if(!U)return;Y(0);const I=window.setInterval(()=>Y(ae=>ae+1),1e3);return()=>window.clearInterval(I)},[U]);const Re=()=>{const I=h.trim();I&&(i(null,I),v(""))},tt=async()=>{if(!b){i(null,ne[0]||"");return}if(!ne.length||C)return;z(!0),E.current=!1;const I=[];for(let ae=0;ae<ne.length&&!E.current;ae++){M(e.evaluating(ae+1,ne.length,ne[ae]));const oe=await Wn(r,ne[ae],"atelier2");if(I.unshift({name:oe.company||ne[ae],verdict:oe.verdict,reason:oe.reason||"",remaining:oe.remaining??null}),F([...I]),oe.verdict==="quota"){I.unshift({name:"",verdict:"quota",reason:e.quota(ne.length-ae-1),remaining:0}),F([...I]),y(ne.slice(ae+1).join(`
`)),M(""),z(!1);return}}M(""),y(E.current?ne.slice(I.length).join(`
