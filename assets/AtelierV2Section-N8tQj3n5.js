...
`)}),y(!0))},[]);h.useEffect(()=>{if(!_)return;se(0);const P=window.setInterval(()=>se(J=>J+1),1e3);return()=>window.clearInterval(P)},[_]);const Me=()=>{const P=g.trim();P&&(i(null,P),b(""))},_e=async()=>{if(!v){i(null,Q[0]||"");return}if(!Q.length||S)return;C(!0),R.current=!1;const P=[];for(let J=0;J<Q.length&&!R.current;J++){F(e.evaluating(J+1,Q.length,Q[J]));const ne=await Mn(r,Q[J],"atelier2");if(P.unshift({name:ne.company||Q[J],verdict:ne.verdict,reason:ne.reason||"",remaining:ne.remaining??null}),k([...P]),ne.verdict==="quota"){P.unshift({name:"",verdict:"quota",reason:e.quota(Q.length-J-1),remaining:0}),k([...P]),w(Q.slice(J+1).join(`
