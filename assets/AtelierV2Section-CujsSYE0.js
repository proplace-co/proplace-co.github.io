...
`)}),S(!0))},[]);m.useEffect(()=>{if(!I)return;Ae(0);const F=window.setInterval(()=>Ae(Z=>Z+1),1e3);return()=>window.clearInterval(F)},[I]);const me=m.useMemo(()=>{const F=E.trim().toLowerCase();return!F||!v?null:h.find(Z=>$e(Z.name)===F&&Ce(Z)===1)||null},[E,h,v]),he=()=>{const F=E.trim();if(F){if(me){u(me);return}i(null,F),g&&y("")}},De=async()=>{if(!g){i(null,C[0]||"");return}if(!C.length||M)return;D(!0),G.current=!1;const F=[];for(let Z=0;Z<C.length&&!G.current;Z++){R(e.evaluating(Z+1,C.length,C[Z]));const se=await pr(r,C[Z],"atelier2");if(F.unshift({name:se.company||C[Z],verdict:se.verdict,reason:se.reason||"",remaining:se.remaining??null}),O([...F]),se.verdict==="quota"){F.unshift({name:"",verdict:"quota",reason:e.quota(C.length-Z-1),remaining:0}),O([...F]),j(C.slice(Z+1).join(`
